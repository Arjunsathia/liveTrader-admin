...
function DepositsPage() {
  ...
  const kpis = [
    { label: 'Total Deposits',  value: depositsData.length, Icon: ArrowDownLeft, accent: 'var(--cyan)',     sub: 'all deposits',              trend: '+5.4%',          trendUp: true  },
    { label: 'Pending',         value: depositsData.filter((d) => d.status === 'PENDING').length,  Icon: Clock,        accent: 'var(--warning)',   sub: 'waiting for review',    trend: 'Requires action', trendUp: false },
    { label: 'Approved',        value: depositsData.filter((d) => d.status === 'APPROVED').length, Icon: CheckCircle2, accent: 'var(--positive)',  sub: 'processed today',       trend: '+8.1%',          trendUp: true  },
    { label: 'Failed',          value: depositsData.filter((d) => d.status === 'FAILED').length,   Icon: XCircle,      accent: 'var(--negative)',  sub: 'failed deposits',       trend: '0 new',          trendUp: true  },
    { label: 'Flagged',         value: depositsData.filter((d) => d.status === 'FLAGGED').length,  Icon: ShieldAlert,  accent: 'var(--negative)',  sub: 'flagged for review',    trend: 'AML Hold',       trendUp: false },
  ...
  const columns = [
    ...
    { key: 'reviewedBy', label: 'Reviewer',  render: (val) => <span className="text-[12px] font-semibold text-text-muted/85">{val}</span> },
    {
          ...
          <div className="flex items-start gap-3 rounded-[12px] border border-negative/20 bg-negative/[0.04] px-4 py-3 shadow-sm animate-in fade-in slide-in-from-top-4 duration-300">
            ...
            <div className="flex-1">
              <h4 className="text-[13px] font-bold text-negative font-heading">
                {depositsData.filter((d) => d.status === 'FLAGGED').length} Deposits Flagged for Review
              </h4>
