...
export function WithdrawMethodSelector({ value, onChange }) {
  ...
  const bankFeeLabel = globalFees.withdrawalFee === '0' || !globalFees.withdrawalFee
    ? 'Free'
    : `$${globalFees.withdrawalFee} flat`;
  ...
  gateways.forEach((g) => {
    ...
    if (g.id === 'stripe') {
      activeMethods.push({
        ...DEFAULT_WITHDRAW_METHODS.card,
        fee: g.fee || '1.5%'
      });
    } else if (g.id === 'swift') {
      activeMethods.push({
        ...DEFAULT_WITHDRAW_METHODS.bank,
        fee: bankFeeLabel,
        processingLabel: `${globalFees.processingDelay || '1'}–3 Business Days`
      });
    } else if (g.id === 'fireblocks') {
      activeMethods.push({
        ...DEFAULT_WITHDRAW_METHODS.crypto,
        fee: '0%'
      });
    } else if (g.id === 'skrill') {
      activeMethods.push({
        ...DEFAULT_WITHDRAW_METHODS.skrill,
        fee: g.fee || '1.0%'
      });
    }
  });

  const methodsToRender = activeMethods.length > 0 ? activeMethods : [DEFAULT_WITHDRAW_METHODS.bank];
