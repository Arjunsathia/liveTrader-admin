...
const SAVED_DESTINATIONS = {
  ...
  skrill: ['Skrill Wallet — skrill.user@email.com'],
};
...
function SummarySidebar({ method, destination, amount, availableBalance, step }) {
  ...
  if (method === 'bank') {
    const flatFee = parseFloat(globalFees.withdrawalFee || 0);
    feeLabel = flatFee > 0 ? `$${flatFee.toFixed(2)}` : 'Free';
  } else if (method === 'card') {
    const pctFee = parseFeePercent(g.fee) || 0.015;
    feeLabel = num > 0 ? `$${(num * pctFee).toFixed(2)} (${(pctFee * 100).toFixed(1)}%)` : `${(pctFee * 100).toFixed(1)}%`;
  } else if (method === 'skrill') {
    const pctFee = parseFeePercent(g.fee) || 0.010;
    feeLabel = num > 0 ? `$${(num * pctFee).toFixed(2)} (${(pctFee * 100).toFixed(1)}%)` : `${(pctFee * 100).toFixed(1)}%`;
  }
...
export function WithdrawPage() {
  ...
  // Find first enabled gateway to set as default method
  const enabledGateways = gateways.filter(g => g.enabled);
  const defaultGateway = enabledGateways[0]?.id || 'swift';
  const defaultMethod = defaultGateway === 'stripe' ? 'card' : defaultGateway === 'swift' ? 'bank' : defaultGateway === 'fireblocks' ? 'crypto' : defaultGateway === 'skrill' ? 'skrill' : 'bank';

  const [step, setStep] = useState(1);
  const [method, setMethod] = useState(defaultMethod);
  const [destination, setDestination] = useState(SAVED_DESTINATIONS[defaultMethod]?.[0] || '');
