...
const FEE_RATES = { card: 0.025, bank: 0, crypto: 0 };
const MIN_AMOUNTS = { card: 10, bank: 50, crypto: 20 };

/**
 * DepositAmountForm
 * Amount input + quick-pick chips + live fee preview + submit button.
 */
export function DepositAmountForm({ method, amount, onChange, onContinue }) {
  const num       = parseFloat(amount) || 0;
  const feeRate   = FEE_RATES[method] ?? 0;
  const minAmount = MIN_AMOUNTS[method] ?? 10;
  const fee       = num * feeRate;
  const netCredit = num - fee;
  const isValid   = num >= minAmount;

  const methodLabels = { card: 'Checkout', bank: 'Bank Details', crypto: 'Generate Address' };
