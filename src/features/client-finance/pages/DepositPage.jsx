...
function SummarySidebar({ method, amount, step }) {
  ...
  const feeRate = m?.id === 'card' ? 0.025 : 0;
            ...
            {step === 1 && (
              <div className="flex flex-col gap-5">
                ...
                >
                  ...
                  Continue with {method === 'card' ? 'Card' : method === 'bank' ? 'Bank Wire' : 'Crypto'}
                </button>
              </div>
            )}
              ...
              <div className="flex flex-col gap-5">
                ...
                {method === 'card' && (
                  ...
                  >
                    ...
                    Proceed to Checkout — ${parseFloat(amount || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                  </button>
                )}
                ...
                {(method === 'bank' || method === 'crypto') && (
                  <button
                    ...
                    className="w-full h-12 rounded-[10px] font-bold text-[13px] transition-all duration-150 cursor-pointer border border-border bg-surface-elevated text-text hover:bg-muted-surface active:scale-95 flex items-center justify-center gap-2"
                  >
                    <Check size={14} strokeWidth={2.5} />
                    I've Sent the Payment — Go to Wallet
                  </button>
                )}
              </div>
            )}
