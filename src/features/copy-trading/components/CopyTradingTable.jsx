...
const renderers = {
  ...
  detail: (val) => (
    <span className="text-[11px] text-text-muted/65 block truncate max-w-[240px]" title={val}>{val}</span>
  ),
};

/**
 * CopyTradingTable — mirrors TradingTable exactly.
 * Maps config columns → DataTable columns using the renderers above,
 * appends an "Open / Manage / Inspect" action column.
 */
  ...
  const columns = [
    ...configCols.map((col) => ({
      ...
      render: (row) => {
        ...
        // user-type renderer needs the whole row for uid
        return renderer ? renderer(col.type === 'user' ? row : val) : <span className="text-[12px] text-text">{val || '—'}</span>;
      },
    })),
    {
      key: 'actions',
