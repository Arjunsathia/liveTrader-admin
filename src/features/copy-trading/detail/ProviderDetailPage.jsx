      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_300px] gap-5">
        <div className="space-y-5">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <KpiCard label="Strategies" value={row.strategies} color="var(--cyan)" Icon={Layers} sub="Total strategies" />
            <KpiCard label="Followers" value={row.followers.toLocaleString()} color="var(--brand)" Icon={Users} sub="All strategies" />
            <KpiCard label="AUM" value={row.aum} color="var(--brand)" Icon={Wallet} sub="Copied capital" />
            <KpiCard label="Win Rate" value={row.winRate} color="var(--positive)" Icon={Target} sub="Across strategies" />
          </div>

          <Card>
            <SH title="Provider Identity" Icon={UserCheck} />
              ...
              <table className="w-full text-[11px]">
                <thead>
                  <tr className="border-b border-white/[0.05]">
                    {['Strategy', 'Followers', 'Win Rate', 'Drawdown', 'Status'].map(h => (
                      <th
                        key={h}
        ...
        <div className="space-y-4 xl:sticky xl:top-[120px]">
          <Card>
            <SH title="Performance Summary" Icon={BarChart2} />
            {[
              ['Win Rate', row.winRate, 'var(--positive)'],
              ['Max Drawdown', row.drawdown, 'var(--negative)'],
              ['AUM', row.aum, 'var(--brand)'],
          ...
          <div className="grid grid-cols-2 gap-2">
            <IBtn label="Flag Review" Icon={ShieldAlert} variant="warning" onClick={() => act('Flagged', row.id)} />
            <IBtn label="View Commissions" Icon={BarChart2} variant="default" onClick={() => act('Commissions', row.id)} />
          </div>
        </div>
      </div>
    </div>
