...
const PAGE = {
  ...
  title: 'Event Logs',
  description: 'Real-time copy engine events, synchronisation failures, and critical system warning indicators.',
};
    ...
    <PageShell>
      <div className="space-y-5 animate-fade-up">
        ...
        <section className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3">
          <KpiCard label="Total Events" value={logsData.length} color="var(--cyan)" Icon={Activity} sub="All events" />
          <KpiCard
            label="Sync Failures"
            value={logsData.filter(l => l.type.includes('SYNC')).length}
            color="var(--negative)"
            Icon={WifiOff}
            sub="Engine sync errors"
            urgent
          />
          <KpiCard
            label="Sub Changes"
          ...
          <div className="flex items-start gap-3 rounded-[12px] border border-negative/20 bg-negative/[0.04] px-4 py-3 shadow-sm animate-in fade-in slide-in-from-top-4 duration-300">
            ...
            <div className="flex-1">
              <h4 className="text-[13px] font-bold text-negative font-heading">
                {logsData.filter(l => l.sev === 'CRITICAL').length} Critical Copy Trading Event
              ...
              <p className="text-[12px] text-negative/80 font-heading mt-0.5 leading-relaxed">
                Check risk flags, sync failures, and provider group connections immediately to prevent account slippage.
              </p>
            </div>
          </div>
        )}
        ...
        <section className="rounded-[12px] border border-border/20 bg-surface-elevated shadow-card-subtle overflow-hidden">
          ...
          <MainTable
            ...
            emptyTitle="No log entries found matching filters."
