...
const PAGE = {
  ...
  description: 'Platform-wide performance statistics, monthly growth rankings, and drawdown analytics.',
};
    ...
    <PageShell>
      <div className="space-y-5 animate-fade-up">
        ...
        <section className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3">
          <KpiCard label="Copied PnL" value={totalCopiedPnl} color="var(--positive)" Icon={TrendingUp} sub="Total follower PnL" />
          <KpiCard label="Monthly Growth" value={monthlyGrowth} color="var(--positive)" Icon={TrendingUp} sub="Platform avg." />
          <KpiCard label="Top Provider" value={bestProvider.provider.split(' ')[0]} color="var(--brand)" Icon={Trophy} sub={`${bestProvider.roi}% ROI`} />
          <KpiCard label="Avg Drawdown" value={`-${avgDD}%`} color="var(--negative)" Icon={TrendingDown} sub="All strategies" />
          <KpiCard label="Success Rate" value={`${successRate}%`} color="var(--cyan)" Icon={Target} sub="Strategies >10% ROI" />
        </section>

        {/* Charts */}
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          <Card>
            <SH title="Monthly Growth Comparison (%)" Icon={TrendingUp} />
          ...
          <Card>
            <SH title="Max Drawdown Comparison (%)" Icon={TrendingDown} />
            <div className="h-[220px]">
              <ResponsiveContainer width="100%" height="100%">
        ...
        <section className="rounded-[12px] border border-border/20 bg-surface-elevated shadow-card-subtle overflow-hidden">
          <div className="px-5 py-4 border-b border-border/12 flex items-center justify-between bg-surface-elevated">
            <SH title="Strategy Performance Ranking" Icon={Trophy} />
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="text-[11px] font-semibold uppercase tracking-[0.08em] text-text-muted/70 border-b border-border/10 bg-bg/20">
                  {['Rank', 'Strategy', 'Provider', 'ROI', 'Win Rate', 'Max DD', 'Followers', 'Trend'].map(h => (
                    <th
                      key={h}
                      className="px-4 py-3"
                    >
