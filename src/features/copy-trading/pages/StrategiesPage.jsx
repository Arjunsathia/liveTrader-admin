...
const PAGE = {
  ...
  description: 'Monitor and manage all copy trading strategies — track AUM, risk thresholds, ROI metrics, and follower activity.',
};
    ...
    <PageShell>
      <div className="space-y-5 animate-fade-up">
        ...
        <section className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
          <KpiCard label="Total Strategies" value={strategiesData.length} color="var(--cyan)" Icon={Layers} sub="All strategies" />
          <KpiCard
            label="Active Providers"
            value={new Set(strategiesData.filter(s => s.status === 'ACTIVE').map(s => s.provider)).size}
            color="var(--positive)"
            Icon={UserCheck}
            sub="With active strategies"
          />
          <KpiCard
            ...
            sub="Across all"
          />
          <KpiCard label="Copied Volume" value="$12.4M" color="var(--brand)" Icon={Copy} sub="Total allocated" />
          <KpiCard label="Avg Win Rate" value={`${avgWin}%`} color="var(--positive)" Icon={Target} sub="All strategies" />
        ...
        <section className="rounded-[12px] border border-border/20 bg-surface-elevated shadow-card-subtle overflow-hidden">
          ...
          <MainTable
            ...
            emptyTitle="No strategies found matching filters."
