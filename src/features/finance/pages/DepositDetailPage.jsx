...
export function DepositDetailPage() {
  ...
  const deposit = depositsData.find((d) => d.id === id) || depositsData[0];
  const [record, setRecord] = useState(deposit);
  ...
  const [localAudit, setLocalAudit] = useState([
    {
      ...
      by: 'System',
      ts: record.created,
      note: `Source: ${record.method}`,
    },
    {
      action: 'Auto-risk assessment completed',
      by: 'RiskEngine',
      ts: record.created,
      note: `Risk level assigned: ${record.risk}`,
    },
  ]);
  ...
  const handleAction = (newStatus, auditMsg) => {
    ...
    toast(`Deposit ${record.id} updated → ${newStatus}`);
  };
  ...
  const handleAddNote = (text) => {
    ...
    setLocalAudit((prev) => [
      ...prev,
      {
        action: 'Internal compliance note added',
        by: 'sys_admin_01',
        ts: new Date().toISOString().replace('T', ' ').slice(0, 16),
        note: text,
      },
  ...
  const avatarHue = ((parseInt(record.id.replace(/\D/g, ''), 10) || 44) * 37) % 360;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
          ...
          <div className="lg:col-span-8 space-y-5">
            ...
            <div className="rounded-[13px] border border-border/18 bg-surface-elevated shadow-card-subtle p-5">
              <SectionHead icon={Hash} title="Transaction Ledger" />
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                <Field label="Transaction ID" value={record.id} mono copyable />
                <Field
                ...
                <Field label="Ledger Method" value={record.method} />
                <Field label="Processor Rail" value={record.rail || 'SWIFT Ledger'} mono />
                <Field label="Timestamp" value={record.created} mono />
                <Field label="Reference Hash" value={record.hash || 'N/A'} mono copyable />
                <Field
                  label="Risk Level"
                  value={record.risk}
                  accent={RISK_CLR[record.risk]}
                />
                <Field label="Reviewed By" value={record.reviewedBy || 'Pending'} />
              </div>
            </div>

            {/* Client Intelligence */}
            <div className="rounded-[13px] border border-border/18 bg-surface-elevated shadow-card-subtle p-5">
              <SectionHead icon={User} title="Client Intelligence" />
