...
function DepositsPage() {
  ...
  const vol24 = depositsData
    .filter((d) => d.status !== 'FAILED')
    .reduce((s, d) => s + d.amtRaw, 0);

  const kpis = [
    { label: 'Total Deposits',  value: depositsData.length, Icon: ArrowDownLeft, accent: 'var(--cyan)',     sub: 'All time',              trend: '+5.4%',          trendUp: true  },
    { label: 'Pending Review',  value: depositsData.filter((d) => d.status === 'PENDING').length,  Icon: Clock,        accent: 'var(--warning)',   sub: 'Awaiting action',       trend: 'Requires action', trendUp: false },
    { label: 'Approved Today',  value: depositsData.filter((d) => d.status === 'APPROVED').length, Icon: CheckCircle2, accent: 'var(--positive)',  sub: 'Processed successfully', trend: '+8.1%',          trendUp: true  },
    { label: 'Failed',          value: depositsData.filter((d) => d.status === 'FAILED').length,   Icon: XCircle,      accent: 'var(--negative)',  sub: 'Payment errors',         trend: '0 new',          trendUp: true  },
    { label: 'Flagged',         value: depositsData.filter((d) => d.status === 'FLAGGED').length,  Icon: ShieldAlert,  accent: 'var(--negative)',  sub: 'AML / compliance hold',  trend: 'AML Hold',       trendUp: false },
    { label: '24h Volume',      value: `$${(vol24 / 1000).toFixed(1)}K`, Icon: TrendingUp, accent: 'var(--brand)', sub: 'Gross deposit volume', trend: '+12.3%',         trendUp: true  },
  ];

  const columns = [
    { key: 'id',         label: 'Deposit ID',   render: (val) => <span className="font-mono text-[12px] font-bold text-brand">{val}</span> },
    ...
    {
      key: 'rail', label: 'Provider',
      render: (val) => (
        <span className="text-[11px] font-semibold border border-border/25 rounded-[4px] px-1.5 py-0.5 text-text-muted/85 font-mono bg-surface-elevated/40">
          {val}
        </span>
      ),
    },
    ...
    { key: 'reviewedBy', label: 'Reviewed By',  render: (val) => <span className="text-[12px] font-semibold text-text-muted/85">{val}</span> },
    {
