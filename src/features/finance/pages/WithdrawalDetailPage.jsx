...
import { STATUS_CLR, RISK_CLR, withdrawalsData } from '@/config/constants/finance/mockData';
  ...
  return (
    <div className="space-y-2">
      <textarea
        ...
        rows={3}
        placeholder="Add compliance note or audit comment..."
...
export function WithdrawalDetailPage() {
  ...
  // Find exact withdrawal or fallback to WDR-DEMO
  const withdrawal = withdrawalsData.find((w) => w.id === id) || withdrawalsData[0];
  const [record, setRecord] = useState(withdrawal);
  const [notes, setNotes] = useState(withdrawal.note ? [withdrawal.note] : []);
  const [toastMsg, setToastMsg] = useState('');
  const [localAudit, setLocalAudit] = useState([
    { action: 'Withdrawal payout requested by user', by: 'ClientPortal', ts: record.created },
    { action: 'Compliance gateway clearance auto-checked', by: 'ComplianceEngine', ts: record.created, note: `Status: ${record.compliance}` },
    { action: 'Sanctions AML list screening completed', by: 'AML_Engine', ts: record.created, note: `Status: ${record.aml}` }
  ]);

  const toast = (msg) => { setToastMsg(msg); setTimeout(() => setToastMsg(''), 3000); };

  const handleAction = (newStatus, auditMsg) => {
  ...
  const handleAddNote = (text) => {
    ...
    setLocalAudit((prev) => [
      ...prev,
      { action: 'Internal compliance audit comment added', by: 'sys_admin_01', ts: new Date().toISOString().replace('T', ' ').slice(0, 16), note: text }
    ]);
    ...
    <PageShell>
      ...
      <div className="space-y-5 animate-fade-up">
        ...
        <div className="flex items-center gap-2 select-none">
          ...
          <span className="text-[11px] text-text-muted/35 font-medium">Finance Ledger</span>
          ...
          <div className="p-6">
                ...
                <div className="flex items-center gap-2">
                  {record.status === 'PENDING' && (
                    <>
                      <button
                        type="button"
                        onClick={() =>
                          handleAction('PAID', 'Withdrawal payout processed successfully')
                        }
                      ...
                      >
                        <CheckCircle2 size={12} /> Approve Payout
                      </button>
                      <button
                        type="button"
                        onClick={() =>
                  ...
                  {(record.status === 'FLAGGED' || record.status === 'FROZEN') && (
                    <>
                      <button
                        type="button"
                        onClick={() =>
                          handleAction('PENDING', 'Compliance hold manually released')
                        }
                      ...
                      >
                        <Play size={12} /> Release Hold
                      </button>
                      <button
                        type="button"
                        onClick={() =>
                          handleAction('FROZEN', 'Escalated to Compliance MLRO')
                        }
                        className="flex items-center gap-1.5 h-8 px-3.5 rounded-[8px] text-[11px] font-bold uppercase tracking-wider bg-negative text-bg hover:brightness-110 active:scale-[0.97] transition-all cursor-pointer shrink-0"
                      >
                        <ArrowUpRight size={12} /> Escalate Case
                      </button>
                    </>
                  )}
                  {record.status === 'FAILED' && (
                    <button
                      type="button"
                      onClick={() =>
                        handleAction('PENDING', 'Retry transaction gateway execution queued')
                      }
            ...
            <div className="mt-5 pt-4 border-t border-border/10 flex items-center gap-0 flex-wrap">
              {[
                ...
                { label: 'Gateway', val: record.reviewedBy || 'Auto System', Icon: Shield },
          ...
          <div className="flex items-start gap-3 rounded-[12px] border border-negative/20 bg-negative/[0.04] px-4 py-3.5 shadow-sm animate-in fade-in slide-in-from-top-4 duration-300">
            ...
            <div className="flex-1">
              <h4 className="text-[12.5px] font-bold text-negative font-heading">
                AML Compliance Flag Active
              </h4>
              <p className="text-[11.5px] text-negative/70 font-heading mt-0.5 leading-relaxed">
                Funds are frozen pending manual compliance clearance. Do not approve payout without rigorous secondary audit checks.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
          ...
          <div className="lg:col-span-8 space-y-5">
            ...
            <div className="rounded-[13px] border border-border/18 bg-surface-elevated shadow-card-subtle p-5">
              <SectionHead icon={Hash} title="Payout Ledger Details" />
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                <Field label="Record ID" value={record.id} mono copyable />
                <Field label="Payment Method" value={record.method} />
                <Field label="Amount Debit" value={`-${record.amount}`} mono accent="var(--negative)" />
                <Field label="Handler Status" value={record.status} accent={statusColor} />
                <Field label="Destination Address" value={record.destination} mono copyable />
                <Field
                ...
                />
                <Field
                  label="AML Status Check"
                  value={record.aml}
                  accent={record.aml === 'CLEAR' ? 'var(--positive)' : 'var(--negative)'}
                />
                <Field label="Creation Time" value={record.created} mono />
                <Field label="Processor / Rail" value={record.rail || 'SWIFT Clearing'} mono />
                <Field label="Assigned Auditor" value={record.reviewedBy || 'Unassigned'} />
              </div>
            </div>

            {/* Beneficiary Client Context */}
            <div className="rounded-[13px] border border-border/18 bg-surface-elevated shadow-card-subtle p-5">
              <SectionHead icon={User} title="Beneficiary Client Context" />
          ...
          <div className="lg:col-span-4 space-y-5">
            ...
            {/* Risk Intelligence */}
            <div className="rounded-[13px] border border-border/18 bg-surface-elevated shadow-card-subtle p-5">
              <SectionHead icon={ShieldAlert} title="Risk Intelligence" />
              <RiskPanel risk={record.risk} amlClear={record.aml === 'CLEAR'} />
            </div>

            {/* Compliance Notes */}
            <div className="rounded-[13px] border border-border/18 bg-surface-elevated shadow-card-subtle p-5">
              <SectionHead icon={FileText} title="Compliance Notes" />
              {notes.length > 0 && (
