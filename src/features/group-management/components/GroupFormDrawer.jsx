...
import { Check, X, AlertTriangle, Layers, Wallet, Shield, TrendingUp } from 'lucide-react';
...
export function GroupFormDrawer({ open, mode, group, onSubmit, onClose }) {
  ...
  const mt5GroupOptions = groupService.listMt5Groups();
    ...
    <MainDrawer open={open} width="max-w-[720px]" onClose={onClose}>
      <div className="flex h-full w-full flex-col overflow-hidden">
            ...
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[9.5px] font-black uppercase tracking-[0.22em] text-brand/65 leading-none">
                  Group Policy Form · {isEdit ? 'Edit Mode' : 'Create Mode'}
                </span>
              </div>
              <h2 className="text-[22px] font-bold tracking-[-0.025em] text-text leading-none">
                {isEdit ? 'Edit Dealing Group' : 'Create Dealing Group'}
              </h2>
              <p className="text-[12px] text-text-muted/50 mt-2 leading-relaxed max-w-[460px]">
                Define user-friendly account tiers and map them directly to MetaTrader 5 server policies.
              </p>
            </div>
            <button
              type="button"
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4 no-scrollbar">
          ...
          <Section step={1} icon={Layers} title="Technical Configuration">
            <PGrid>
              <div>
                <PField
                  label="Group Name"
          ...
          <Section step={2} icon={Wallet} title="Financial Boundaries">
            <PGrid>
              <PSelect
                label="Currency Unit"
              ...
              />
              <PField
                label="Minimum Subsequent Deposit"
                type="number"
              ...
              />
              <PField
                label="Daily Max Withdrawal Cap"
                type="number"
          ...
          <Section step={3} icon={Shield} title="Execution Policies">
            <PGrid>
              <PSelect
          ...
          <Section step={4} icon={TrendingUp} title="Leverage Limits">
            <PGrid>
              ...
              />
              <PField
                label="Max Allowed Accounts / User"
                type="number"
        ...
        <div className="flex-shrink-0 border-t border-border/15 bg-surface-elevated">
          {!isValid && (
            <div className="flex items-center gap-2.5 mx-6 mt-4 rounded-[9px] border border-warning/22 bg-warning/6 px-3.5 py-2.5">
              ...
              <span className="text-[11.5px] font-medium text-warning leading-tight">
                Group name and first deposit requirements are required.
              </span>
            </div>
          )}
          ...
          <div className="flex items-center justify-between gap-3 px-6 py-4">
            <p className="text-[10px] text-text-muted/35 font-medium select-none">
              <span className="text-brand/60 font-black">*</span> Required policies
            </p>
