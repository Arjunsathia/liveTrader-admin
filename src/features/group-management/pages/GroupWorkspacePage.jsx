...
export function GroupWorkspacePage() {
  ...
  const handleDeleteGroup = (id) => {
    ...
    if (window.confirm(`Are you sure you want to delete the dealing group "${target.name}"?`)) {
      groupService.delete(id);
      handleRefresh();
      triggerToast(`Dealing group "${target.name}" deleted successfully.`);
    }
  };
  ...
  const handleFormSubmit = (formData) => {
    if (drawerMode === 'edit' && editingGroup) {
      ...
      triggerToast(`Dealing group "${formData.name}" successfully updated.`);
    } else {
      groupService.create(formData);
      triggerToast(`Dealing group "${formData.name}" successfully created.`);
    }
    setDrawerOpen(false);
    handleRefresh();
  };
    ...
    <PageShell>
      ...
      <div className="space-y-5.5">
        ...
        <header className="flex flex-col gap-2.5">
          <div>
            ...
            <p className="text-[13.5px] text-text-muted/80 mt-2 leading-relaxed max-w-2xl select-none">
              Configure broker-facing account tiers and define their MT5 server routing, deposit limits, leverages, and trading policies in a single centralized workspace.
            </p>
          </div>
        </header>
        ...
        <div className="flex gap-1 border-b border-white/[0.06] overflow-x-auto no-scrollbar pb-px mb-2 select-none">
          {[
            { id: 'directory', label: 'Group Directory', Icon: Layers },
            { id: 'mappings', label: 'MT5 Account Mappings', Icon: Link },
          ].map((tab) => {
