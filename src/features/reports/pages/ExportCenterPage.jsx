...
function AddTemplateDrawer({ open, onClose, onSave }) {
  ...
  return (
    <MainDrawer open={open} onClose={onClose} width="max-w-[720px]">
      <DrawerHeader title="Create Export Template" eyebrow="New Automation" onClose={onClose} />
      <DrawerBody>
        <div className="space-y-6">
          {/* Hint */}
          <div className="flex items-start gap-3 rounded-[12px] border border-brand/15 bg-brand/[0.04] px-4 py-3.5">
            ...
            <p className="text-[11.5px] text-text-muted/70 font-heading leading-relaxed">
              Automated templates run on a recurring schedule and dispatch reports to configured recipients immediately on completion.
            </p>
          </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-start gap-3 rounded-[12px] border border-warning/15 bg-warning/[0.04] px-4 py-3">
                ...
                <p className="text-[11px] text-warning/72 font-heading leading-relaxed">
                  Templates activate immediately on creation. Verify recipients to prevent delivery failure.
                </p>
              </div>
            </div>
          </DrawerSection>
      ...
      <DrawerFooter>
        <div className="flex items-center justify-between gap-4 w-full">
          <span className="text-[10px] text-text-muted/38 leading-snug max-w-[240px] font-heading">
            Template will be active immediately and run on schedule.
          </span>
          <div className="flex items-center gap-2">
...
export function ExportCenterPage() {
  ...
  const act = (msg, id = '') => {
    const msgs = {
      'All active templates triggered': 'Triggered real-time generation for all active templates.',
      'Triggered': `Manually dispatched job run for template: ${id}`,
      'Paused': `Paused automated export schedule: ${id}`,
      'Resumed': `Resumed automated export schedule: ${id}`,
      'Deleted': `Archived export template: ${id}`,
      'Edit opened': `Template editor loaded: ${id}`,
      'Retried': 'Initiated retry for failed job run.',
      'Dismissed': 'Alert log dismissed.',
    };
    ...
    <PageShell>
      <div className="space-y-5 animate-fade-up">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div>
            ...
            <p className="text-[13.5px] text-text-muted/80 mt-2 leading-snug max-w-xl">
              Configure automated report delivery, recurring cron schedules, and audit failure histories.
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { label: 'Active Templates', val: activeCount, color: 'var(--positive)', Icon: Layers },
            { label: 'Paused Templates', val: pausedCount, color: 'var(--warning)', Icon: Pause },
            ...
            <Panel>
              ...
              <div className="px-4 pt-3.5 pb-2">
                ...
                >
                  ...
                  <div>
                    <p className="text-[11px] font-bold text-text/78 font-heading leading-none">Automation Enabled</p>
                    <p className="text-[9.5px] text-text-muted/42 font-heading mt-0.5">All scheduled jobs running normally.</p>
                  </div>
                </div>
              </div>
                ...
                {exportFailureLog.length > 0
                  ...
                  : (
                    <div className="flex flex-col items-center py-8 px-4">
                      ...
                      <p className="text-[11px] font-bold text-text-muted/35 font-heading">No failures logged</p>
                      <p className="text-[10px] text-text-muted/22 font-heading mt-0.5">All exports are running cleanly.</p>
                    </div>
                  )}
              </div>
            </Panel>
