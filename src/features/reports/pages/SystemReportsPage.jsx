...
const PAGE = {
  ...
  description: 'Automated server jobs, database vacuum monitors, and background webhook delivery synchronization traces.',
};
...
function SystemReportsPage() {
  ...
  const act = (msg, id) => {
    let message = '';
    if (msg === 'Downloaded') message = `Successfully downloaded report log: ${id}`;
    else if (msg === 'Retried') message = `Retriggered system cron job for ID: ${id}`;
    else if (msg === 'Deleted') message = `Archived system log: ${id}`;
    else if (msg === 'Viewed') message = `Loaded detail trace analyzer for system job ${id}`;
    else if (msg === 'Job triggered') message = `Manual operational job triggered. Syncing system logs...`;
    else if (msg === 'Bulk retry') message = `Queued all failed system jobs for bulk re-execution.`;
    else if (msg === 'Exported') message = `Bulk exports generated for all system logs matching query.`;
    else message = `${msg}: ${id}`;
  ...
  const columns = [
    ...
    { key: 'name', label: 'Report / Job Name', render: (val) => <span className="text-[12.5px] font-semibold text-text/90 max-w-[240px] block truncate hover:text-primary transition-colors">{val}</span> },
