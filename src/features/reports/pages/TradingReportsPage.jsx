...
const PAGE = {
  ...
  description: 'Execution logs, P&L summaries, and high-frequency volume performance audits.',
};
...
function TradingReportsPage() {
  ...
  const act = (msg, id) => {
    let message = '';
    if (msg === 'Downloaded') message = `Successfully downloaded report file: ${id}`;
    else if (msg === 'Retried') message = `Retriggered report generation job for ID: ${id}`;
    else if (msg === 'Deleted') message = `Archived report record: ${id}`;
    else if (msg === 'Generate') message = `Trading Report Generation console initialized successfully.`;
    else if (msg === 'Exported') message = `Bulk exports generated for all trading reports matching search.`;
    else message = `${msg}: ${id}`;
  ...
  const columns = [
    ...
    { key: 'drawdown', label: 'Drawdown', render: (val) => val !== '—' ? <span className="font-mono text-negative font-bold text-[11.5px]">{val}</span> : <span className="text-text-muted/30">—</span> },
