  ...
  return (
    <div className="space-y-5.5">
      <SettingsSection
        ...
        desc="Integrate credit card processors, cryptocurrency rails, bank wire templates, and specify general transaction fees."
      />
      ...
      {activeTab === 'gateways' && (
        <div className="space-y-3.5">
              ...
              >
                ...
                {isExpanded && (
                  <div className="px-5 pb-5 pt-2.5 border-t border-border/10 animate-in slide-in-from-top-1.5 duration-200">
                    <FGroup cols={2}>
                      <div>
                        <FieldLabel hint="Processor authentication key code">API Key (Read-only)</FieldLabel>
                        <TInput value={`pk_live_••••••••${gw.id}`} readOnly mono />
                      </div>
                      <div>
                        <FieldLabel hint="Callback target matching API webhooks setup">Webhook URL</FieldLabel>
                        <TInput value={`https://api.live-trader.com/webhooks/${gw.id}`} readOnly mono />
                      </div>
                      <div>
                        <FieldLabel hint="Specific transaction fee override rules">Fee Override</FieldLabel>
                        <TInput value={gw.fee} onChange={() => { }} mono />
                      </div>
                      <div>
                        <FieldLabel hint="Currencies supported by checkout">Settlement Currencies</FieldLabel>
                    ...
                    <div className="mt-4.5 rounded-[8px] border border-border/15 bg-bg px-4 py-1">
                      <ToggleRow
                        ...
                        desc="Redirect transacting traffic through sandbox verification — no actual funds swapped"
                        val={gw.testMode}
                        onChange={() => handleToggleGateway(gw.id, 'testMode')}
                      />
                      <ToggleRow
                        label="Auto-Settle Batches"
                        desc="Commit batch settlements automatically on a 24h schedule"
                        val={true}
                        onChange={() => { }}
                      />
                      <ToggleRow
                        label="Enforce 3D Secure Protection"
                        desc="Mandate card verification checks via secondary bank handshakes"
                        val={gw.id === 'stripe'}
                        onChange={() => { }}
                      />
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-4.5">
                      <Btn Icon={Wifi} label="Test Connection" variant="cyan" small onClick={() => alert(`${gw.name} communication active.`)} />
                      <Btn Icon={Edit2} label="Modify Parameters" variant="default" small onClick={() => { }} />
                      {gw.enabled ? (
                        <Btn Icon={Lock} label="Deactivate Gateway" variant="danger" small onClick={() => handleToggleGateway(gw.id, 'enabled')} />
                      ) : (
                        <Btn Icon={Unlock} label="Activate Gateway" variant="primary" small onClick={() => handleToggleGateway(gw.id, 'enabled')} />
                      )}
                    </div>
                  </div>
                )}
          ...
          >
            <Plus size={14} /> Connect New Gateway
          </button>
        </div>
      )}
      ...
      {activeTab === 'fees' && (
        <div className="space-y-5">
          <SettingsCard
            ...
            desc="Configure checkout charge structures applied globally across all methods unless explicitly overridden."
            Icon={BadgePercent}
          >
            <FGroup cols={2}>
              <div>
                <FieldLabel hint="Default percent charge applied to client deposit tickets">Default Deposit Fee</FieldLabel>
                <TInput
              ...
              <div>
                <FieldLabel hint="Default percent charge applied to withdrawal orders">Default Withdrawal Fee</FieldLabel>
                <TInput
              ...
              <div>
                <FieldLabel hint="Minimum limits acceptable for withdrawal tickets">Min Withdrawal Limit</FieldLabel>
                <TInput
              ...
              <div>
                <FieldLabel hint="Maximum daily cumulative withdrawal limits per client account">Max Withdrawal Limit (Daily)</FieldLabel>
                <TInput
              ...
              <div>
                <FieldLabel hint="Standard waiting time before withdrawal tickets are settled">Settlement Window</FieldLabel>
                <TSelect
              ...
              <div>
                <FieldLabel hint="Base default currency used for general fee operations">Standard Fee Currency</FieldLabel>
                <TSelect
      ...
      {activeTab === 'priority' && (
        <div className="space-y-5">
          <WarnBanner
            severity="info"
            title="Automated Routing Priority Rules"
            message="Priority governs default router path selection. If a client selects an checkout method supported by multiple active providers, the higher-priority active pathway launches."
          />

          <SettingsCard
            title="Processor Priority Order"
            desc="Arrange priority coordinates. Highest coordinate launched first."
            Icon={ArrowUpDown}
          >
