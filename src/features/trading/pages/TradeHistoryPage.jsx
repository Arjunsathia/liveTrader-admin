...
function TradeHistoryPage() {
  ...
  const columns = [
    ...
    { key: 'openPrice', label: 'Open Px', render: (val) => <span className="font-mono text-[12px] text-text-muted/75">{val}</span> },
    { key: 'closePrice', label: 'Close Px', render: (val) => <span className="font-mono text-[12px] text-text-muted/75">{val}</span> },
    {
      key: 'pnl',
      label: 'P&L',
      render: (val) => {
