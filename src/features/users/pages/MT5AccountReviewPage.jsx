...
export function MT5AccountReviewPage() {
  ...
  const [localAudit, setLocalAudit] = useState([
    { action: 'Dealing terminal instance registered on bridge', by: 'SystemEngine', ts: originalAccount.lastSync || '2026-05-26 12:00' },
  ...
  const handleSaveSettings = () => {
    ...
    setLocalAudit(prev => [
      ...prev,
      { action: 'Operational state modified on desk controls', by: 'sys_admin_01', ts: new Date().toISOString().replace('T', ' ').slice(0, 16), note: `Leverage: ${leverage} | Status: ${status}` }
    ]);
    triggerToast(`Terminal #${account.login} configuration successfully deployed`);
  };
  ...
  const handleForceSync = () => {
    ...
    setTimeout(() => {
      ...
      setLocalAudit(prev => [
        ...prev,
        { action: 'Cluster handshake forced sync', by: 'sys_admin_01', ts: syncedTime }
      ]);
      triggerToast(`Live cluster metrics synchronized for #${account.login}`);
    }, 1000);
  };
  ...
  const handleResetPassword = () => {
    setLocalAudit(prev => [
      ...prev,
      { action: 'Trader terminal password reset queued', by: 'sys_admin_01', ts: new Date().toISOString().replace('T', ' ').slice(0, 16) }
    ]);
    triggerToast(`Password reset instruction queued for #${account.login}`);
  };
      ...
      <div className="space-y-5 animate-fade-up">
        ...
        <DetailPanel className="relative">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-6 pt-7 bg-surface-bright/5">
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-text-muted/70 mb-1.5 font-heading">MetaTrader 5 Bridge Review</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2 space-y-5">
            {/* Account Snapshot */}
            <DetailPanel>
              <PanelHead title="Terminal Profile Parameters" subtitle="Core broker-bridge settings & mapping metrics" />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-5">
                <InfoRow label="Login ID" value={account.login} mono copyable />
                <InfoRow label="Active Server Node" value={account.server} mono />
                <InfoRow label="Desk Group Mapping" value={account.group || 'retail_usd_std'} mono copyable />
                <InfoRow label="Margin Account Type" value={account.type || 'Live'} />
                <InfoRow label="Base Account Currency" value={account.currency || 'USD'} />
                <InfoRow label="Telemetric Sync Stamp" value={account.lastSync} mono />
              </div>
            </DetailPanel>

            {/* Capital & Leverage Metrics */}
            <DetailPanel>
              <PanelHead title="Terminal Capital Balance & Margins" subtitle="Operational balance sheets currently active on desk gateway" />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-5">
                <InfoRow label="Available Balance" value={account.balance} mono accent="var(--cyan)" />
                <InfoRow label="Gateway Real-Time Equity" value={account.equity || account.balance} mono accent="var(--brand)" />
                <InfoRow label="Active margin allocation" value={account.margin || '$0.00'} mono accent="var(--warning)" />
                <InfoRow label="Net free trading margin" value={account.freeMargin || account.balance} mono accent="var(--positive)" />
                <InfoRow
                  label="Dynamic Margin Level"
            ...
            <DetailPanel>
              <PanelHead title="Dealing Desk History & Audits" subtitle="Chronological bridge actions logged on this MT5 terminal" />
              <div className="p-6">
          ...
          <div className="space-y-5">
            ...
            <DetailPanel>
              <PanelHead title="Gateways & Desk Control" subtitle="Deploy adjustments directly onto cluster node" />
              <div className="p-5 space-y-4">
                <div className="flex flex-col gap-1.5">
                  <label className="text-[11px] font-bold uppercase tracking-[0.05em] text-text-muted/70 select-none font-heading">Adjust Account Leverage</label>
                  <select
                    value={leverage}
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-[11px] font-bold uppercase tracking-[0.05em] text-text-muted/70 select-none font-heading">Operational Status</label>
                  <select
                    value={status}
                ...
                >
                  <Sliders size={12} /> Apply Bridge Configuration
                </button>
              </div>
            </DetailPanel>
            ...
            {/* Quick Actions */}
            <DetailPanel>
              <PanelHead title="Administrative Actions" subtitle="Fast telemetric terminal triggers" />
              <div className="p-5 space-y-2.5">
                ...
                >
                  <RefreshCw size={12} className={isSyncing ? 'animate-spin' : ''} /> {isSyncing ? 'Syncing...' : 'Force Sync telemetries'}
                </button>
                ...
                >
                  <KeyRound size={12} /> Reset Password credentials
                </button>
              </div>
            </DetailPanel>
          </div>
